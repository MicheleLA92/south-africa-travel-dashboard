...
GOLF_MAP_ICON_URL = "https://cdn.jsdelivr.net/gh/twitter/twemoji@14.0.2/assets/72x72/26f3.png"
CANOE_MAP_ICON_URL = "https://cdn.jsdelivr.net/gh/twitter/twemoji@14.0.2/assets/72x72/1f6f6.png"
...
def photo_stop_icon_url(stop):
    ...
    if stop.get("kind") == "art":
        return ART_MAP_ICON_URL
    if stop.get("kind") == "golf":
        return GOLF_MAP_ICON_URL
    if stop.get("kind") == "canoe":
        return CANOE_MAP_ICON_URL
    if stop.get("kind") == "hiking":
        return HIKING_MAP_ICON_URL
    if stop.get("kind") == "monkey":
        return MONKEY_MAP_ICON_URL
    if stop.get("kind") == "running":
        return RUNNING_MAP_ICON_URL
...
def render_route_map_and_timeline(data, selected_photo_stop_name=None, map_key="route_photo_map"):
    ...
    st.caption("Tatsächlich gefahrene Route · kleine Symbole: 📷 Foto · 🍴 Restaurant · 🏡 Unterkunft · 🍷 Aktivität · 🎨 Art/Decor · ⛳ Golf · 🛶 Kanufahrt · 🥾 Hiking · 🐒 Monkeyland · 🏃‍♀️ Running · 🐘 Safari · 🐧 Pinguine. Symbol antippen, um Fotos zu öffnen.")
...
def render_photo_map(data, map_key="photo_map"):
    ...
    st.caption("📷 Foto-Spot · 🍴 Restaurant/Food · 🏡 Unterkunft · 🍷 Aktivität · 🎨 Art/Decor · ⛳ Golf · 🛶 Kanufahrt · 🥾 Hiking · 🐒 Monkeyland · 🏃‍♀️ Running · 🐘 Safari · 🐧 Pinguine · Symbol antippen, um die Fotos zu öffnen.")
